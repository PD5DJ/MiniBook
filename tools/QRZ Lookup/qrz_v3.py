...
MY_LOCATOR = "JO22LO"  # Fixed grid locator
...
def on_query():
    ...
    if not callsign:
#        messagebox.showerror("Missing Callsign", "Please enter a callsign to query.")
        return
...
#        messagebox.showinfo("Result", "No data found for this callsign.")
#        return
...
# Callsign input
tk.Label(root, text="Enter Callsign:").grid(row=2, column=0, sticky="e")
entry_callsign = tk.Entry(root, width=20)
entry_callsign.grid(row=2, column=1)
...
sp_lp_label.grid(row=len(fields) + 3, column=0, columnspan=2, pady=5)

# Buttons
tk.Button(root, text="Run Query", command=on_query).grid(row=len(fields) + 4, column=0, pady=10)
tk.Button(root, text="Exit", command=root.quit).grid(row=len(fields) + 4, column=1, pady=10)
